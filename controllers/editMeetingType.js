...
module.exports.editMeetingType = (req, res,next) => {
    if (req.userType !== "Visitor") {
        ...
        if (docs.length == 0){
            ...
            if (comparison === 0) {
                ...
                try{
                    meetingModel.findOneAndUpdate({_id: req.params.meetingId}, {type: req.body.type },function(err){
                        if (err) throw err;
                        res.json("Type of Meeting is  Updated Successfuly")
                     next();
                    });
