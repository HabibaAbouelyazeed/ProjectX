...
module.exports.editMeetingTime= (req, res,next) => {
    if (req.userType !== "Visitor") {
        ...
        if (docs.length == 0){
            ...
            if (comparison === 0) {
                ...
                try{
                    meetingModel.findOneAndUpdate({_id: req.params.meetingId}, {dateAndTime: new Date() },function(err){
                        if (err) throw err;
                        res.json("Time Updated Successfuly")
                     next();
                    });
